...
func TestFlattenEntitlementAttributes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: types.SetValueMust(
				entitlementAttributeObjectType,
				[]attr.Value{
					types.ObjectValueMust(
						entitlementAttributeObjectType.AttrTypes,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := flattenEntitlementAttributes(ctx, tt.input, &diags)
			...
			if !got.Equal(tt.want) {
				t.Fatalf(
					"flattenEntitlementAttributes() = %#v, want %#v",
					got, tt.want,
				)
			}
