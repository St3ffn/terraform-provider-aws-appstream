...
func (r *resource) Delete(ctx context.Context, req tfresource.DeleteRequest, resp *tfresource.DeleteResponse) {
	...
	if state.Name.IsNull() || state.Name.IsUnknown() {
		resp.Diagnostics.AddError(
			...
			"Cannot delete imported image because name must be known.",
		)
		return
	}
	...
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error Deleting AWS AppStream Imported Image",
			fmt.Sprintf("Could not delete imported image %q: %v", name, err),
		)
		return
	}
}
