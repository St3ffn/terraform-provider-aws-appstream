...
func (r *resource) Read(ctx context.Context, req tfresource.ReadRequest, resp *tfresource.ReadResponse) {
	...
	newState, diags := r.readImportedImage(ctx, state)
...
func (r *resource) readImportedImage(ctx context.Context, prior resourceModel) (*resourceModel, diag.Diagnostics) {
	...
	if err != nil {
		...
		diags.AddError(
			"Error Reading AWS AppStream Imported Image",
			fmt.Sprintf("Could not read imported image %q: %v", name, err),
		)
		return nil, diags
	}
	...
	state := &resourceModel{
		...
		AgentSoftwareVersion:        prior.AgentSoftwareVersion,
		DisplayName:                 util.StringOrNull(image.DisplayName),
		Description:                 util.StringOrNull(image.Description),
		RuntimeValidationConfig:     prior.RuntimeValidationConfig,
		AppCatalogConfig:            prior.AppCatalogConfig,
