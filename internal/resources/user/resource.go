...
// ImportState expects <authentication_type>|<user_name> and writes both fields
// plus id into state.
func (r *resource) ImportState(ctx context.Context, req tfresource.ImportStateRequest, resp *tfresource.ImportStateResponse) {
	authenticationType, userName, err := parseID(req.ID)
	if err != nil {
		resp.Diagnostics.AddError(
			"Unexpected Import Identifier",
			"Expected import identifier format: <authentication_type>|<user_name>",
		)
		return
	}
