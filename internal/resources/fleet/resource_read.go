...
func (r *resource) readFleet(ctx context.Context, prior model) (*model, diag.Diagnostics) {
	...
	state := &model{
		...
		ImageName:                      util.FlattenOptionalComputedString(prior.ImageName, fleet.ImageName),
		ImageARN:                       util.FlattenOptionalComputedString(prior.ImageARN, fleet.ImageArn),
		...
		MaxUserDurationInSeconds:       util.FlattenStateOwnedInt32(prior.MaxUserDurationInSeconds, fleet.MaxUserDurationInSeconds),
		DisconnectTimeoutInSeconds:     util.FlattenStateOwnedInt32(prior.DisconnectTimeoutInSeconds, fleet.DisconnectTimeoutInSeconds),
		IdleDisconnectTimeoutInSeconds: util.FlattenStateOwnedInt32(prior.IdleDisconnectTimeoutInSeconds, fleet.IdleDisconnectTimeoutInSeconds),
		Description:                    util.StringOrNull(fleet.Description),
		DisplayName:                    util.StringOrNull(fleet.DisplayName),
		EnableDefaultInternetAccess:    util.FlattenStateOwnedBool(prior.EnableDefaultInternetAccess, fleet.EnableDefaultInternetAccess),
