...
// Update reconciles mutable fields for updated images.
// Updated images are treated as immutable artifacts, so update only applies tags.
func (r *resource) Update(ctx context.Context, req tfresource.UpdateRequest, resp *tfresource.UpdateResponse) {
	...
	if plan.ARN.IsNull() || plan.ARN.IsUnknown() {
		resp.Diagnostics.AddError(
			...
			"Cannot update updated image because arn must be known.",
		)
		return
	}
